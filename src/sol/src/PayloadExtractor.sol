...
contract PayloadExtractor {
     ...
     */
    function parsePayload(bytes memory payload) public pure returns (ParsedPayload memory parsed) {
     ...
     */
    function extractTestAddress(bytes memory payload) public pure returns (address) {
        require(payload.length >= 75, "Payload too short");
     ...
     * @return The original address (with bytes reversed back)
     */
    function extractRecipientAddress(bytes memory payload) public pure returns (address) {
     ...
     */
    function extractAndReverseAddress(bytes memory payload, uint256 offset) internal pure returns (address) {
     ...
     */
    function extractAndReverseUint16(bytes memory payload, uint256 offset) public pure returns (uint16) {
                            ...
                            or(
                                or(
                                    or(
                                        or(
                                            or(
                                                or(shl(152, and(temp, 0xff)), shl(144, and(shr(8, temp), 0xff))),
